...
#define SECRET_SSID     "PIVOT_DX"
#define SECRET_PASS     "Pivot@PivotDX"
...
boolean fuoco = false;
volatile boolean set_reset = false;
volatile boolean va_bene = false;
boolean bandierina_fault = false;
boolean bandierina_emergenza = false;
boolean bandierina_emergenza_risolta = false;
int tempo_emergenza = 0; 
boolean bandierina_fuoco = false;
boolean bandierina_Bea = false;
boolean bandiera_pulsanti = false;
volatile int segno = -1;  // a differenza della SX  qui metto il segno vediamo se basta cambiarlo solo qui ( motore ed encoder rimangono uguali)
                          // la discriminante per il pos potrebbe essere il senso di rotazione e non più l'interno esterno ... son cazzate
